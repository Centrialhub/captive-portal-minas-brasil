-- Retention is a single transaction. No cleanup runs when this migration is applied.
-- Keep privileged implementation outside the exposed API schema.
CREATE SCHEMA IF NOT EXISTS captive_internal;
REVOKE ALL ON SCHEMA captive_internal FROM PUBLIC, anon, authenticated;
GRANT USAGE ON SCHEMA captive_internal TO service_role;
CREATE INDEX captive_auth_operation_retention ON public.captive_auth_operations(completed_at,id)
  WHERE status IN ('confirmed','rejected','expired_unconfirmed');
CREATE INDEX captive_auth_attempt_orphan_retention ON public.captive_auth_attempts(created_at,id)
  WHERE auth_operation_id IS NULL AND captive_session_id IS NULL;

CREATE FUNCTION captive_internal.retirable_auth_operations(p_now timestamptz)
  ...
  WHERE o.status IN ('confirmed','rejected','expired_unconfirmed')
    ...
    AND NOT EXISTS (
      ...
      WHERE m.operation_id=o.id AND (
        ...
        OR a.created_at >= p_now - CASE WHEN s.status='authorized' THEN interval '365 days' ELSE interval '180 days' END
...
CREATE FUNCTION captive_internal.retirable_captive_sessions(p_now timestamptz, p_operations uuid[])
...
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = '' AS $$
  ...
  WHERE (s.auth_operation_id=ANY(p_operations) OR (
    ...
    AND (a.id IS NULL OR (a.auth_operation_id IS NULL
      AND a.created_at < p_now-CASE WHEN s.status='authorized' THEN interval '365 days' ELSE interval '180 days' END
...
CREATE FUNCTION captive_internal.captive_housekeeping(p_dry_run boolean, p_batch_size integer, p_actor_user_id uuid)
...
BEGIN
  ...
  EXECUTE 'SELECT coalesce(array_agg(a.id),''{}''::uuid[]) FROM (
    ...
    WHERE a.auth_operation_id IS NULL AND a.captive_session_id IS NULL
      AND a.created_at < $1-CASE WHEN a.status=''authorized'' THEN interval ''365 days'' ELSE interval ''180 days'' END
...
CREATE FUNCTION public.captive_housekeeping(p_dry_run boolean DEFAULT true, p_batch_size integer DEFAULT 100, p_actor_user_id uuid DEFAULT NULL)
RETURNS jsonb LANGUAGE sql SECURITY INVOKER SET search_path = '' AS $$
  SELECT captive_internal.captive_housekeeping(p_dry_run,p_batch_size,p_actor_user_id);
$$;
REVOKE ALL ON FUNCTION captive_internal.retirable_auth_operations(timestamptz),
  captive_internal.retirable_captive_sessions(timestamptz,uuid[]),
  captive_internal.captive_housekeeping(boolean,integer,uuid),
  public.captive_housekeeping(boolean,integer,uuid) FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION captive_internal.captive_housekeeping(boolean,integer,uuid),
  public.captive_housekeeping(boolean,integer,uuid) TO service_role;
