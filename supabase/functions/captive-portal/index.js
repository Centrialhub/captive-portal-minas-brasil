...
function supabaseAuth(authHeader) {
...
function isValidPhone(phone) {
    ...
    if (digits.length === 10 && !/^[2-5]/.test(digits.slice(2, 3)))
        return false;
    return true;
}
function isValidCPF(cpf) {
    ...
    if (firstDV !== parseInt(digits[9], 10))
        return false;
    const secondDV = calcDV(digits.slice(0, 10), [11, 10, 9, 8, 7, 6, 5, 4, 3, 2]);
    if (secondDV !== parseInt(digits[10], 10))
        return false;
    return true;
}
/**
 ...
 */
async function syncWithClubeMais(lead, db, traceId) {
    ...
    try {
        ...
        console.error(`[clubemais] Sync exception: ${err.message}`);
...
function logEvent(db, args) {
    ...
    db.from("portal_events").insert(row).then(() => { }, (e) => console.warn("[logEvent] insert failed:", e?.message));
    if (args.session_id) {
        const patch = {
            last_step: args.step,
            ...(args.session_patch || {}),
        ...
        if (args.trace_id)
            patch.trace_id = args.trace_id;
        if (args.status === "error") {
            if (args.error_code)
                patch.last_error_code = args.error_code;
            if (args.error_message)
                patch.last_error_message = args.error_message.slice(0, 500);
        }
        db.from("captive_sessions").update(patch).eq("id", args.session_id).then(() => { }, (e) => console.warn("[logEvent] session patch failed:", e?.message));
    }
}
// ========== Detect Store ==========
...
async function detectStoreFromRequest(db, req, apMac) {
    const fallback = {
        store_id: null,
        store_slug: "geral",
        redirect_url: null,
    ...
    if (normApMac.length === 12) {
        ...
        if (store?.is_active) {
            ...
            db.from("store_access_points")
                ...
                .eq("ap_mac", normApMac)
                .then(() => { }, (e) => console.warn("[ap-mac] last_seen update failed:", e?.message));
...
async function discoverStoreByClientMac(db, clientMac, apMacHint) {
    if (!clientMac || clientMac.length !== 12)
        return null;
    ...
    const probes = stores.map(async (store) => {
        ...
        try {
            ...
            if (!match)
                return null;
            return { store, apMac: match.ap_mac || null };
        }
        catch (e) {
            console.warn(`[discover] probe ${store.slug} failed:`, e?.message);
            return null;
        }
        finally {
            httpClient?.close();
    ...
    if (apToPersist.length === 12) {
        db.from("store_access_points")
            ...
            .then(() => console.log(`[discover] persisted ap ${apToPersist} -> ${winner.store.slug}`), (e) => console.warn(`[discover] persist ap failed:`, e?.message));
    }
    return {
        store_id: winner.store.id,
...
async function checkRateLimitDb(db, key, windowSeconds, maxHits, blockSeconds = 0) {
    try {
...
function isDuplicate(key) {
    ...
    if (last && now - last < DEDUP_WINDOW_SEC * 1000)
        return true;
...
setInterval(() => {
    ...
    for (const [key, ts] of dedupMap) {
        if (now - ts > DEDUP_WINDOW_SEC * 2000)
            dedupMap.delete(key);
    }
}, 30000);
...
async function enrichGeoIp(db, ip) {
    ...
    if (cached && cached.last_geoip_at) {
        const ageHours = (Date.now() - new Date(cached.last_geoip_at).getTime()) / 3600000;
    ...
    await db.from("origin_ip_clusters").upsert({ public_ip: ip, last_seen_at: new Date().toISOString() }, { onConflict: "public_ip", ignoreDuplicates: false });
    return { city: null, region: null, country: null, isp: null, asn: null, source: "none" };
}
async function incrementClusterLeadCount(db, ip) {
    try {
        const { data } = await db
            .from("origin_ip_clusters")
            .select("lead_count")
        ...
        console.warn("Failed to increment cluster lead_count:", e.message);
    }
}
async function getWhatsappConfig(db, _storeId) {
    ...
    if (!data)
        return null;
    if (!data.whatsapp_webhook_enabled || !data.whatsapp_webhook_url)
        return null;
    return {
...
function createUnifiHttpClient() {
    if (!UNIFI_CA_CERT)
        return null;
 ...
 */
async function unifiTryLogin(loginUrl, httpClient, username, password) {
    ...
    try {
        ...
        try {
            ...
            if (httpClient)
                warmOpts.client = httpClient;
            const warmRes = await fetch(`${baseUrl}/`, warmOpts);
            ...
            console.log(`[UniFi] Warmup GET failed (non-fatal): ${e.message}`);
        }
        // ---- POST login ----
        const headers = { ...baseHeaders };
        if (warmupCookies)
            headers["Cookie"] = warmupCookies;
        if (warmupCsrf)
            headers["X-CSRF-Token"] = warmupCsrf;
        const payload = {
        ...
        if (httpClient)
            fetchOpts.client = httpClient;
        const res = await fetch(loginUrl, fetchOpts);
        clearTimeout(timeout);
 ...
 */
async function unifiLogin(baseUrl, httpClient, username, password) {
...
async function unifiFetchStations(staUrl, headers, httpClient) {
    const ac = new AbortController();
    ...
    try {
        ...
        clearTimeout(t);
        return { ok: false, error: err.name === "AbortError" ? "/stat/sta timeout" : err.message };
    }
}
/**
 ...
 */
function pickEffectiveMac(stations, portalMacFormatted, // aa:bb:cc:dd:ee:ff
apMac, ssid) {
    const target = portalMacFormatted.toLowerCase();
    const exact = stations.find((s) => (s.mac || "").toLowerCase() === target);
    if (exact)
        return { mac: target, remapped: false, candidateCount: 1 };
    ...
    const candidates = stations.filter((s) => {
        ...
        if (ssid && s.essid && s.essid !== ssid)
            return false;
        if (typeof s.assoc_time === "number" && s.assoc_time < cutoff)
            return false;
        return true;
    });
 ...
 */
async function unifiSendAuthorizeCmd(url, headers, httpClient, payload) {
    const ac = new AbortController();
    const timeout = setTimeout(() => ac.abort(), UNIFI_TIMEOUT_MS);
    try {
        const fetchOpts = {
        ...
        return {
            ok: false, status: 0,
            error: err.name === "AbortError" ? `Timeout after ${UNIFI_TIMEOUT_MS}ms` : err.message,
        };
    }
}
/**
 ...
 */
async function unifiAuthorizeByMac(controllerUrl, siteId, clientMac, username, password, options = {}) {
    ...
    const buildHeaders = (login) => {
        ...
        if (login.isUnifiOs && login.token) {
            ...
            h["Cookie"] = login.csrfToken
                ...
                : `unifises=${login.cookie}`;
            if (login.csrfToken)
                h["X-Csrf-Token"] = login.csrfToken;
        }
        return h;
    };
    let closed = false;
    const closeClient = () => { if (!closed) {
        closed = true;
        try {
            httpClient?.close();
        }
    ...
    try {
        ...
        if (!login.ok) {
            closeClient();
            return { ok: false, reason: "UNIFI_LOGIN_FAILED", error: `UniFi login failed: ${login.error}` };
        }
        ...
        if (stationsRes.sessionExpired) {
            ...
            if (login.ok) {
                ...
                stationsRes = await unifiFetchStations(staUrl0, headers, httpClient);
            }
        }
        ...
        const buildPayload = (mins) => {
            const p = { cmd: "authorize-guest", mac: effectiveMac, minutes: mins };
            if (apMacForPayload)
                p.ap_mac = apMacForPayload.toLowerCase();
            return p;
        };
        let activeUrl = "";
        let lastError = "";
        let cmdSentAt = 0;
        let cmdAcceptedAtIso;
        let usedMinutes = desiredMinutes;
        let policyOverride = false;
        const sendOnce = async (mins) => {
            for (const url of stamgrUrls) {
                ...
                if (cmd.sessionExpired) {
                    ...
                    if (login.ok) {
                        headers = buildHeaders(login);
                        cmd = await unifiSendAuthorizeCmd(url, headers, httpClient, buildPayload(mins));
                    }
                }
                if (cmd.ok && cmd.rcOk) {
                    activeUrl = url;
        ...
        const pollConfirmation = async () => {
            ...
            let lastVerifySnapshot = { mac: effectiveMac, found: false };
            try {
                for (let attempt = 1; attempt <= VERIFY_BACKOFF_MS.length; attempt++) {
                    ...
                    if (staRes.sessionExpired) {
                        ...
                        if (login.ok) {
                            headers = buildHeaders(login);
                            staRes = await unifiFetchStations(staUrl, headers, httpClient);
                        }
                    }
...
async function unifiAuthorizeWithRetry(controllerUrl, siteId, mac, username, password, options = {}) {
    ...
    for (let attempt = 0; attempt <= UNIFI_RETRY_COUNT; attempt++) {
        ...
        if (last.reason === "MAC_RANDOMIZATION_AMBIGUOUS")
            return { ...last, attempts: attempt + 1 };
...
async function authorizeClient(db, storeId, storeSlug, clientMac, sessionId, clientIp, context = {}) {
    if (!storeId) {
    ...
    const siteId = store.unifi_site_id || "default";
    const result = await unifiAuthorizeWithRetry(store.unifi_controller_url, siteId, clientMac, storeUser, storePass, { apMac: context.apMac || null, ssid: context.ssid || null, minutes: desiredMinutes, fastReturn: !!context.fastReturn });
    // Persist UniFi audit columns regardless of outcome
    const auditUpdate = {};
    if (result.cmd_accepted_at)
        auditUpdate.unifi_cmd_accepted_at = result.cmd_accepted_at;
    if (result.last_verify_result)
        auditUpdate.unifi_last_verify_result = result.last_verify_result;
    if (result.ok) {
 ...
 */
function getControllerBaseForGuestRedirect(controllerUrl) {
...
logEvent(db, {
    ...
    trace_id: traceId, store_id: resolvedStoreId,
...
async function internalHousekeeping(db) {
    const now = new Date();
...
async function handleAdminSettings(req) {
    ...
    if (req.method === "PUT") {
        ...
        if (body.whatsapp_webhook_url !== undefined) {
            ...
            if (url && !url.startsWith("https://"))
                return errorResponse("URL deve começar com https://");
        ...
        if (typeof body.whatsapp_webhook_secret === "string") {
            ...
            if (secret.length > 0 && secret.length < 8)
                return errorResponse("Secret deve ter pelo menos 8 caracteres");
...
async function handleAdminStores(req) {
    const auth = await requireAdmin(req);
    if (auth instanceof Response)
        return auth;
    const { db } = auth;
    ...
    if (req.method === "POST") {
        ...
        if (!name)
            return errorResponse("Nome obrigatório");
    ...
    if (req.method === "PUT") {
        const body = await safeParseJson(req);
        if (!body || !isValidUUID(body.id))
            return errorResponse("Missing or invalid store id");
        const updateData = {};
        ...
        if (body.city !== undefined)
            updateData.city = sanitizeString(body.city, 100);
        if (body.is_active !== undefined)
            updateData.is_active = !!body.is_active;
        if (body.post_auth_redirect_url !== undefined)
            updateData.post_auth_redirect_url = sanitizeString(body.post_auth_redirect_url, 500);
        if (body.unifi_site_id !== undefined)
            updateData.unifi_site_id = sanitizeString(body.unifi_site_id, 100);
        if (body.unifi_controller_url !== undefined)
            updateData.unifi_controller_url = sanitizeString(body.unifi_controller_url, 500);
    ...
    if (req.method === "DELETE") {
        ...
        if (!body || !isValidUUID(body.id))
            return errorResponse("Missing or invalid store id");
        const { error } = await db.from("stores").delete().eq("id", body.id);
        if (error)
            return errorResponse(error.message, 500);
        await db.from("audit_logs").insert({
            store_id: body.id,
...
async function handleAdminLeads(req, url) {
    const auth = await requireAdmin(req);
    if (auth instanceof Response)
        return auth;
    const { db } = auth;
    ...
    if (from)
        query = query.gte("created_at", from.length === 10 ? `${from}T00:00:00.000Z` : from);
    if (to)
        query = query.lte("created_at", to.length === 10 ? `${to}T23:59:59.999Z` : to);
    if (format === "csv") {
...
async function handleAdminConsent(req) {
    const auth = await requireAdmin(req);
    if (auth instanceof Response)
        return auth;
    const { db } = auth;
    ...
    if (req.method === "POST") {
        ...
        if (!version)
            return errorResponse("version é obrigatória");
        if (!text)
...
async function handleAdminSessions(req, url) {
    const auth = await requireAdmin(req);
    if (auth instanceof Response)
        return auth;
    const { db } = auth;
...
async function handleAdminClusters(req, url) {
    const auth = await requireAdmin(req);
    if (auth instanceof Response)
        return auth;
    const { db } = auth;
    ...
    if (from)
        query = query.gte("last_seen_at", from.length === 10 ? `${from}T00:00:00.000Z` : from);
    if (to)
        query = query.lte("last_seen_at", to.length === 10 ? `${to}T23:59:59.999Z` : to);
    if (format === "csv") {
...
async function handleAdminStoreIps(req, url) {
    const auth = await requireAdmin(req);
    if (auth instanceof Response)
        return auth;
    const { db } = auth;
    ...
    if (req.method === "POST") {
        ...
        if (!isValidUUID(body.store_id))
            return errorResponse("store_id inválido");
        const ip = sanitizeString(body.public_ip, 45);
        if (!ip)
            return errorResponse("public_ip obrigatório");
        const { data, error } = await db.from("store_public_ips")
            .insert({ store_id: body.store_id, public_ip: ip, is_active: body.is_active !== false })
    ...
    if (req.method === "DELETE") {
        const body = await safeParseJson(req);
        if (!body || !isValidUUID(body.id))
            return errorResponse("Missing or invalid id");
        const { error } = await db.from("store_public_ips").delete().eq("id", body.id);
        if (error)
            return errorResponse(error.message, 500);
        return jsonResponse({ ok: true });
    }
...
async function handleAdminAccessPoints(req, url) {
    const auth = await requireAdmin(req);
    if (auth instanceof Response)
        return auth;
    const { db } = auth;
    ...
    if (req.method === "POST") {
        ...
        if (body.action === "import_from_controller") {
            ...
            const { data: store } = await db.from("stores")
                ...
                .maybeSingle();
            if (!store?.unifi_controller_url)
                return errorResponse("Loja sem controladora configurada");
            const ctrlUrl = store.unifi_controller_url.replace(/\/+$/, "");
            const user = UNIFI_USERNAME;
            const pass = UNIFI_PASSWORD;
            if (!user || !pass) {
            ...
            try {
                ...
                const opts = { method: "GET", headers, redirect: "manual" };
                if (httpClient)
                    opts.client = httpClient;
                const rDev = await fetch(`${baseUrl}/api/s/${siteId}/stat/device`, opts);
                const devList = await rDev.json().catch(() => null);
                const aps = Array.isArray(devList?.data)
                    ? devList.data.filter((d) => d.type === "uap" && typeof d.mac === "string")
                    : [];
                ...
                if (rows.length === 0)
                    return jsonResponse({ imported: 0, message: "Nenhum AP (uap) encontrado na controladora" });
        ...
        if (!macRaw)
            return errorResponse("ap_mac obrigatório");
    ...
    if (req.method === "DELETE") {
        ...
        if (apMac.length !== 12)
            return errorResponse("ap_mac inválido");
...
async function handleTestUnifiReach(req) {
    const auth = await requireAdmin(req);
    if (auth instanceof Response)
        return auth;
    const { db } = auth;
    const body = await safeParseJson(req);
    const controllerUrl = body?.controller_url || "";
    ...
    if (!targetUrl && body?.store_slug) {
        const { data: store } = await db.from("stores")
            ...
            .eq("slug", body.store_slug).maybeSingle();
        targetUrl = store?.unifi_controller_url || "";
    }
    if (!targetUrl)
        return errorResponse("controller_url ou store_slug obrigatório");
...
async function handleTestAuthorize(req) {
    const auth = await requireAdmin(req);
    if (auth instanceof Response)
        return auth;
    const { db } = auth;
    ...
    if (!storeSlug)
        return errorResponse("store_slug obrigatório");
    if (!mac || !isValidMac(mac))
        return errorResponse("MAC inválido (ex: AA:BB:CC:DD:EE:FF)");
    const { data: store } = await db.from("stores")
...
async function handleAdminLeadsXml(req, url) {
    const auth = await requireAdmin(req);
    if (auth instanceof Response)
        return auth;
    const { db } = auth;
    ...
    if (storeSlug)
        xml += `  <store_slug>${escapeXml(storeSlug)}</store_slug>\n`;
    ...
    for (const lead of rows) {
        ...
        if (lead.cpf)
            xml += `    <cpf>${escapeXml(lead.cpf)}</cpf>\n`;
        if (lead.email)
            xml += `    <email>${escapeXml(lead.email)}</email>\n`;
        if (lead.phone)
            xml += `    <phone>${escapeXml(lead.phone)}</phone>\n`;
        if (lead.client_mac)
            xml += `    <client_mac>${escapeXml(lead.client_mac)}</client_mac>\n`;
        if (lead.origin_ip)
            xml += `    <origin_ip>${escapeXml(lead.origin_ip)}</origin_ip>\n`;
        if (lead.origin_city)
            xml += `    <origin_city>${escapeXml(lead.origin_city)}</origin_city>\n`;
        if (lead.origin_region)
            xml += `    <origin_region>${escapeXml(lead.origin_region)}</origin_region>\n`;
...
async function handleHousekeeping(req) {
    const auth = await requireAdmin(req);
    if (auth instanceof Response)
        return auth;
    const { db } = auth;
...
// ========== Housekeeping (Cron) ==========
async function handleCronHousekeeping(req) {
...
async function handlePortalHtml(req, url) {
...
async function handleClientEvent(req) {
...
function extractAuthContext(body) {
    return {
...
async function authorizeAuthenticatedUser(args) {
    ...
    try {
        ...
        console.warn("[auth] Idempotency check failed (ignoring):", e.message);
    }
    ...
    try {
        ...
        console.warn("[auth] Atomic lock RPC failed (falling back to standard flow):", e.message);
    }
    ...
    try {
        ...
        console.warn("[auth] lead upsert failed:", e.message);
    }
    logEvent(db, {
        session_id: sessionId,
        trace_id: traceId,
    ...
    const authResult = await authorizeClient(db, storeId, storeSlug, ctx.clientMac, sessionId, clientIp || "", { apMac: ctx.apMac, ssid: ctx.ssid, fastReturn: false });
    return {
        session_id: sessionId,
...
async function handleRequestPasswordReset(req) {
    const db = supabaseAdmin();
...
async function handleAuthorizeExisting(req) {
    const db = supabaseAdmin();
    const clientIp = getPublicIp(req);
    const ua = req.headers.get("user-agent") || "";
    const body = await safeParseJson(req);
    ...
    if (!profile) {
        ...
        logEvent(db, {
            ...
            payload: { provider: userRes.user.app_metadata?.provider, email: emailValue },
            client_ip: clientIp,
        });
    }
    ...
    const provider = String(userRes.user.app_metadata?.provider || "").toLowerCase();
    const authMethod = hint === "google" || provider === "google" ? "google" :
        hint === "apple" || provider === "apple" ? "apple" :
            "silent";
    // Check if CPF is required for Google users before UniFi authorization
    if (authMethod === "google") {
        const isCpfPending = !profile?.cpf_digits || profile?.cpf_required === true;
        if (isCpfPending) {
            logEvent(db, {
    ...
    if (result.authorized && profile?.cpf_digits && profile?.full_name && profile?.phone_digits) {
        const bgSync = (async () => {
            try {
                ...
                console.warn("[authorize-existing] CRM sync failed (bg):", e.message);
            }
        })();
        // @ts-ignore
    ...
    if (cpfDigits && name && phoneDigits) {
        const bgSync = (async () => {
            try {
                ...
                console.warn("[update-profile] ClubeMais sync failed (bg):", e.message);
            }
        })();
        // @ts-ignore
        if (typeof EdgeRuntime !== "undefined" && EdgeRuntime?.waitUntil) {
...
Deno.serve(async (req) => {
    ...
    try {
        ...
        if ((path === "/" || path === "" || path === "/portal" || path === "/portal/" ||
        ...
        if (path === "/client-event" && req.method === "POST")
            return await handleClientEvent(req);
        ...
        if (path === "/request-password-reset" && req.method === "POST")
            return await handleRequestPasswordReset(req);
        if (path === "/authorize-existing" && req.method === "POST")
            return await handleAuthorizeExisting(req);
        // 3. Admin endpoints (requires service_role/admin auth)
        if (path === "/admin/settings")
            return await handleAdminSettings(req);
        if (path === "/admin/stores")
            return await handleAdminStores(req);
        if (path === "/admin/store-ips")
            return await handleAdminStoreIps(req, url);
        if (path === "/admin/access-points")
            return await handleAdminAccessPoints(req, url);
        if (path === "/admin/leads-xml" && req.method === "GET")
            return await handleAdminLeadsXml(req, url);
        if (path === "/admin/leads")
        ...
        if (path === "/admin/clusters")
            return await handleAdminClusters(req, url);
        if (path === "/admin/test-authorize" && req.method === "POST")
            return await handleTestAuthorize(req);
        if (path === "/admin/test-unifi-reach" && req.method === "POST")
            return await handleTestUnifiReach(req);
        if (path === "/admin/housekeeping" && req.method === "POST")
            return await handleHousekeeping(req);
        // 4. System endpoints
        if (path === "/cron/housekeeping" && req.method === "POST")
            return await handleCronHousekeeping(req);
 ...
 */
async function validateOAuthAttempt(db, attemptId, token) {
...
async function handleOAuthInit(req) {
    ...
    if (!rl.allowed)
        return errorResponse("Muitas tentativas. Aguarde alguns minutos.", 429);
